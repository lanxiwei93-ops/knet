param(
    [string]$Source = "D:\sqlite\sqlite3.exe",
    [string]$TargetRelativePath = "tools\sqlite\sqlite3.exe"
)
...
if (Test-Path -LiteralPath $targetPath -PathType Leaf) {
    ...
    if ($existingTargetHash -eq $sourceHash) {
        Write-Output "SQLite already deployed to: $targetPath"
...
Write-Output "SQLite deployed to: $targetPath"
